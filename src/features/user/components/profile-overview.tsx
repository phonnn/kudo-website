    ...
    <>
      <section className="profile-summary card">
        ...
        <div className="profile-identity">
          ...
          <h1>{user.data?.name ?? "Loading…"}</h1>
          <p>Your recognition points and reward activity in one place.</p>
        </div>
        <div className="points-balance">
          <span>Available points</span>
          <strong>{points.data?.earned ?? "—"}</strong>
      ...
      <section className="profile-stats" aria-label="Point summary">
        <div className="card stat">
          ...
          <strong>{points.data?.lifetimeEarned ?? "—"}</strong>
        </div>
        <div className="card stat">
          <span>Lifetime redeemed</span>
          <strong>{points.data?.lifetimeRedeemed ?? "—"}</strong>
        </div>
        <div className="card stat">
          <span>Rewards claimed</span>
          <strong>{redemptions.data?.length ?? "—"}</strong>
        </div>
      </section>
      <section className="history-section">
        ...
        {redemptions.isPending ? (
          <div className="card empty">Loading redemptions…</div>
          ...
          <div className="card empty">You have not redeemed a reward yet.</div>
        )}
      </section>
    </>
  );
