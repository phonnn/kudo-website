...
export function SendKudoForm() {
  ...
  return (
    <form className="card composer" onSubmit={submit}>
      ...
      <label>
        Core value
        <select name="coreValue" defaultValue="Own the outcome">
          <option>Own the outcome</option>
          <option>Customer first</option>
      ...
      {sendKudo.error && (
        <p className="error">
          {sendKudo.error instanceof ApiError ? sendKudo.error.message : "Something went wrong."}
        </p>
      )}
      <button disabled={sendKudo.isPending}>
        {sendKudo.isPending ? "Sending…" : "Send recognition"}
      </button>
    </form>
  );
}
